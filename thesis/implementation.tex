...
\begin{enumerate}
    \item Während der Kalibrierung der Vive legt der Nutzer den Spielebereich (die sogenannte \emph{Play~ Area}) fest.
    ...
    \autoref{fig:environment_setup} verdeutlicht den Grund dafür.
...
\begin{figure}
    \includegraphics[width=\textwidth]{figures/environment_setup}
    \caption{Setup des virtuellen Umgebung. %
        Links: Skizze der Unity-Szene. %
        Das \texttt{Player}-Objekt \enquote{schaut} zu Beginn entlang der z-Achse. %
        Rechts: Bei der SteamVR-Kalibrierung muss die Play Area die gleiche Ausrichtung haben wie das \texttt{Player}-Objekt.}
