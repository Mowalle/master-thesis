...
Wie zu erkennen ist, liegen die Mittelwerte von $3D_l$~(-3,15 $\pm$ \ang{10,17}), $3D_h$~(-2.87 $\pm$ \ang{10,45}) und $2D$~(-3,35 $\pm$ \ang{7,81}) nahe beieinander.
...
Eine Analyse der Ausreißer zeigt, dass 4 von 8 Ausreißern (\SI{50}{\percent}) von Iterationen auf Karte~4 stammen.
\autoref{fig:horizontal_offset_per_map} zeigt die durchschnittlichen horizontalen Abweichungen zwischen den einzelnen Karten.
Dabei sind signifikante Unterschiede in den Abweichungen vorhanden ($\chi^2(6) = \num{11.701}, p < \num{0.05}$).
Die horizontale Abweichung in Karte~4 ($\num{-6.04} \pm \ang{12.3}$) ist signifikant größer als in Karte~1 ($\num{-2.28} \pm \ang{5.58}, z = \num{-2.182}, p = \num{0.029}$), Karte~2 ($\num{-1.02} \pm \ang{5.92}, z = \num{-3.007}, p < \num{0.005}$) und Karte~5 ($\num{-1.1} \pm \ang{8.24}, z = \num{-2.57}, p  = \num{0.01}$).
Außerdem ist die Abweichung in Karte~3 ($\num{-5.26} \pm \ang{12.72}$) signifikant größer als in Karte~2 ($\num{-1.02} \pm \ang{5.92}, z = \num{-2.157}, p = \num{0.031}$).

Weiterhin ist eine Tendenz zu erkennen, dass die Richtungsschätzungen \enquote{nach links} ausschlagen (negativer Winkel).
Zum einen sind die Mittelwerte der horizontalen Abweichungen über alle Karten und Konditionen negativ.
Zum anderen weichen von den insgesamt 252~Richtungsschätzungen 158 (\SI{62,7}{\percent}) nach links ab.

\begin{figure}
    \centering
    \includegraphics[height=0.45\textheight]{figures/analysis/horizontal_offset_per_map}
    \caption{Horizontale Abweichung in $\degree$ (Grad) bei der Richtungsschätzung pro Karte.}
